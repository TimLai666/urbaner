...
add_bmc_block(s, x3, margin_y, col3_w, top_h, "④ 價值主張 VP", [
    ...
    ("• 美日自家旗艦皆 Beard/Mustache 精品", False, 8.5),
    ("• 雙市場差異化主打自家 SKU", False, 8.5),
    ("  US: B0GL2DKVQH (Beard)", False, 8.5),
    ("  JP: B07CYZH2XC (Beard)", False, 8.5),
...
S_items = [
    ...
    ("S4  美日 Beard/Mustache 自家旗艦 Hero SKU 已成形", True, 10),
...
so_items = [
    ...
    ("[S4×O2] 集中 Hero SKU B0GL2DKVQH，做 Father's Day 禮盒版", True, 9.5),
...
main_strat = [
    ...
    ("    ：美日 Beard 自家旗艦年營收 > USD 5M 後再啟動", False, 8.5),
...
add_bmc_block(s, x2, margin_y2 + half_h2 + gap, col2_w, half_h2, "③ KR", [
    ("• Hero SKU B0GL2DKVQH (Beard)", False, 8),
...
add_bmc_block(s, x3 + Inches(0.05), margin_y2 + top_h2 + gap, Inches(6.31), bot_h2,
              "⑨ 收益流", [
    ("• 主：Beard / Mustache Trimmer 系列銷售 (Hero B0GL2DKVQH)", False, 8),
...
US_S = [
    ...
    ("S2  Beard 自家旗艦 Hero SKU B0GL2DKVQH 已成形", True, 10),
    ("    距理想 2.49 (市場頂尖 Ufree 1.60、仍需追趕)；4,360 則 US 自有評論", False, 8.5),
...
us_so = [
    ...
    ("[S2×O4] B0GL2DKVQH 升級成 7-in-1 套組版主推 Q4", True, 9.5),
...
us_main = [
    ...
    ("", False, 5),
    ("策略描述：", True, 10),
    ("• 主推 B0GL2DKVQH (Beard 自家旗艦) 改為 7-in-1 套組 + 禮盒包裝", False, 9.5),
...
add_bmc_block(s, x2, margin_y2 + half_h2 + gap, col2_w, half_h2, "③ KR", [
    ("• Hero SKU B07CYZH2XC (Beard/Mustache)", False, 8),
...
add_bmc_block(s, x3 + Inches(0.05), margin_y2 + top_h2 + gap, Inches(6.31), bot_h2,
              "⑨ 收益流", [
    ("• 主：Beard/Mustache Trimmer 系列 (自家旗艦 Hero B07CYZH2XC)", False, 8),
    ("  Nose/Ear 為市場頂尖所在、屬待新開類別", False, 8),
...
JP_S = [
    ...
    ("S3  Beard 自家旗艦 Hero SKU B07CYZH2XC 已成形", True, 10),
    ("    距理想 3.43 (市場頂尖 1.97、JP 落後較大)；7,163 則 JP 自有評論", False, 8.5),
...
JP_O = [
    ...
    ("    URBANER 需新開 Nose/Ear 旗艦、以中價精品切入", False, 8.5),
]
JP_T = [
...
jp_so = [
    ...
    ("[S3×O3] B07CYZH2XC 加 dB 數値化 (50dB 以下)", True, 9.5),
    ("        靜音切入點", False, 8.5),
...
jp_main = [
    ...
    ("  自家旗艦 Hero SKU B07CYZH2XC (Beard) → 補 W2", False, 9.5),
...
key_lines = [
    ...
    ("②  Hero SKU 先做深、再做廣  — US: B0GL2DKVQH / JP: B07CYZH2XC；不要分散資源", False, 10),
...
stages = [
    ("Now", "0-30 天", "立即執行 (低成本、高槓桿)", COLOR_RED, [
        ...
        ("【US】Hero SKU B0GL2DKVQH Listing A+ 加入", True, 10),
    ...
    ("Later", "90 天 +", "策略性投入", COLOR_GREEN, [
        ...
        ("    對沖美中關稅風險", False, 9),
        ("【整體】美日 Beard 自家旗艦年營收 > USD 5M 後", True, 10),
